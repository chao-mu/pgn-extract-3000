...
static const uint64_t *en_passant_section =
    Random64 + (NUM_SQUARES * NUM_PIECES) + 4;
// Offset from the en_passant_section by the number of en-passant columns.
static const uint64_t *white_to_move_element =
    Random64 + (NUM_SQUARES * NUM_PIECES) + 4 + 8;
 ...
 */
static int piece_id(char c) { return strchr(FEN_pieces, c) - FEN_pieces; }
...
uint64_t piece_hash(char piece, int rank, int col) {
  return piece_section[64 * piece_id(piece) + (8 * (rank - FIRSTRANK)) +
                       (col - FIRSTCOL)];
}
