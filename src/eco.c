    ...
    if (EcoTable[ix] != NULL) {
      ...
      for (entry = EcoTable[ix]; entry != NULL; entry = entry->next) {
        fprintf(stderr, "%s %lu %lu ", entry->ECO_tag,
                entry->required_hash_value, entry->cumulative_hash_value);
      }
      fprintf(stderr, "\n");
    }
  }
 ...
 * a game.
 */
static int eco_match_level(EcoLog *entry, HashCode current_hash_value,
 ...
 * Currently unused.
 */
static int eco_match_quality(EcoLog *entry, HashCode current_hash_value,
                             HashCode cumulative_hash_value,
...
void save_eco_details(const Game *game_details, const Board *final_position,
  ...
  for (entry = EcoTable[ix]; (entry != NULL) && can_save; entry = entry->next) {
    if ((entry->required_hash_value == game_details->final_hash_value) &&
      ...
      const char *tag = entry->ECO_tag, *opening = entry->Opening_tag,
  ...
  if (can_save) {
    ...
    if (game_details->tags[OPENING_TAG] != NULL) {
      if ((last_entry != NULL) && (last_entry->Opening_tag != NULL) &&
          (strcmp(last_entry->Opening_tag, game_details->tags[OPENING_TAG]) ==
           0)) {
        /* Share the last entry's tag. */
