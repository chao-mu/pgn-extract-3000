 ...
 */
static void init_list_of_known_tags(void) {
  unsigned i;
          ...
          if (extract_yytext(symbol_start, linep)) {
            ...
            if (move_seems_valid(yytext)) {
              ...
              while (epi < num_ep_strings &&
                     strncmp((const char *)symbol_start, ep[epi],
                             strlen(ep[epi])) != 0) {
                epi++;
              }
...
TokenType skip_to_next_game(TokenType token) {
  if (skip_token(token)) {
    ...
    do {
      if (token == COMMENT) {
        /* Free the space. */
        if ((yylval.comment != NULL) && (yylval.comment->comment != NULL)) {
 ...
 */
static FILE_LIST list_of_files = {(const char **)NULL, (SourceFileType *)NULL,
                                  0, 0};

/* Return the index number of the current input file in list_of_files. */
unsigned current_file_number(void) { return current_file_num; }
...
static void fill_input_buffer(FILE *fpin) {
  if (!feof(fpin)) {
    input_buffer_limit =
        fread(input_buffer, sizeof(*input_buffer), INPUT_BUFFER_LEN, fpin);
  } else {
...
void add_filename_list_from_file(FILE *fp, SourceFileType file_type) {
  if ((list_of_files.files == NULL) || (list_of_files.max_files == 0)) {
    ...
    list_of_files.files = (const char **)malloc_or_die((INIT_LIST_SPACE + 1) *
                                                       sizeof(const char *));
    list_of_files.file_type = (SourceFileType *)malloc_or_die(
        (INIT_LIST_SPACE + 1) * sizeof(SourceFileType));
...
void add_filename_to_source_list(
  ...
  if (list_of_files.num_files == list_of_files.max_files) {
    ...
    if ((list_of_files.files == NULL) || (list_of_files.max_files == 0)) {
      ...
      list_of_files.files = (const char **)realloc_or_die(
          (void *)list_of_files.files,
          (list_of_files.max_files + MORE_LIST_SPACE + 1) *
              sizeof(const char *));
      list_of_files.file_type = (SourceFileType *)realloc_or_die(
          (void *)list_of_files.file_type,
          (list_of_files.max_files + MORE_LIST_SPACE + 1) *
  ...
  if (file_type == CHECKFILE) {
    ...
    for (location = 0; (location < list_of_files.num_files) &&
                       (list_of_files.file_type[location] == CHECKFILE);
         location++) {
      /* Do nothing. */
    }
...
unsigned long get_line_number(void) { return line_number; }
